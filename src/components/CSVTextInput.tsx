...
export default function CSVTextInput({ onDataLoaded, onError }: CSVTextInputProps) {
      ...
      Papa.parse(csvText, {
        ...
        complete: (results) => {
          try {
            ...
            // Papa.parse with header: true returns results.data as unknown[], but we expect Record<string, unknown>[]
const { tasks, headers } = processParsedData(results.data as Record<string, unknown>[], extractedHeaders);
            if (tasks.length === 0) {
